          ...
          const RECT *rect, int *x)
{
            ...
            && (rect->top != rect->bottom || rect->left != rect->right))
    {
        ...
        else if( rect->top <= line && line <= rect->bottom)
        {
            ...
            x[1] = (line == rect->bottom ? rect->right : PDC_n_cols - 1);
            rval = 1;
        }
        else if( rect->top >= line && line >= rect->bottom)
        {
            x[0] = (line == rect->bottom ? rect->right : 0);
            x[1] = (line == rect->top ? rect->left : PDC_n_cols - 1);
            rval = 1;
        }
    }
    return( rval);
    ...
    if( before.top > -1 || after.top > -1)
        if( memcmp( &after, &before, sizeof( RECT)))
        {
            ...
            else     /* _not_ selecting rectangle; selecting lines */
            {
                ...
                for( line = 0; line < PDC_n_rows; line++)
                {
                    int x[4], n_rects = 0, i;
               ...
               const int xloc, const int yloc, const int menu_shown)
{
    DWORD is_new_key;
    HKEY hNewKey;
    long rval = RegCreateKeyEx( HKEY_LOCAL_MACHINE, _T( "SOFTWARE\\PDCurses"),
...
static int min_lines = 2, max_lines = 400;
static int min_cols = 1, max_cols = 400;

void PDC_set_resize_limits( const int new_min_lines, const int new_max_lines,
                  const int new_min_cols, const int new_max_cols)
{
    min_lines = new_min_lines;
                                     ...
                                     int *xloc, int *yloc, int *menu_shown)
{
    ...
    HKEY hKey;
    long rval = RegOpenKeyEx( HKEY_LOCAL_MACHINE, _T( "SOFTWARE\\PDCurses"),
                        0, KEY_READ, &hKey);
    ...
    if( rval == ERROR_SUCCESS)
    {
        ...
        if( rval == ERROR_SUCCESS)
        {
            _stscanf( data, _T( "%dx%d,%d,%d,%d,%d"), &x, &y, &PDC_font_size,
                                            xloc, yloc, menu_shown);
            if( n_cols)
                *n_cols = x;
            if( n_rows)
...
static void HandleSyskeyDown( WPARAM wParam, LPARAM lParam,
               int *ptr_modified_key_to_return )
{
    ...
    /* Ignore keys with Alt and Ctrl pressed.   On non-US */
    /* keyboards, this usually means Alt-Gr is pressed and   */
    /* we'll get some sort of meaningful WM_CHAR message. */
    if( key >= KEY_MIN)
                          ...
                          const LPARAM lParam)
{
    ...
    case WM_LBUTTONUP:
        ...
        if( (PDC_mouse_rect.left != PDC_mouse_rect.right ||
             PDC_mouse_rect.top != PDC_mouse_rect.bottom) &&
             (PDC_mouse_rect.right >= 0 && curscr && curscr->_y) )
        {
...
// this function is called by a new thread... or was;  multi-threading
// is no longer needed
...
//      wndclass.style         = CS_HREDRAW | CS_VREDRAW ;
    ...
    PDC_hWnd = CreateWindow( AppName, WindowTitle,
                    (n_default_columns == -1) ?
